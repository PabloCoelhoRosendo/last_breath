...
int main(void) {
    ...
    // REMOVIDO: Sistema de rastreamento de posições anteriores
    // Agora a colisão com mapa é verificada dentro das próprias funções de atualização
    ...
    while (!WindowShouldClose()) {
        ...
        if ((jogador.vida <= 0 || jogador.jogoVencido) && IsKeyPressed(KEY_R)) {
            ...
            // Listas temporárias removidas - colisão agora é tratada nas funções de atualização
        ...
        atualizarJogo(&jogador, &listaZumbis, &listaBalas, mapaAtual);
        ...
        // Atualizar bosses (colisão com mapa agora é tratada dentro de atualizarBoss)
        atualizarBoss(&listaBosses, &jogador, &listaBalas, GetFrameTime(), mapaAtual);
